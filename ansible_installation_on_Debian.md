## Ansible Installation on Debian

The following steps are to install Ansible on Debian. The following code snippets are taken from Windows Subsystem for Linux (WSL):
1. Check Debian OS Release `os-release`:

    ```
    $ cat /etc/os-release
    PRETTY_NAME="Debian GNU/Linux 12 (bookworm)"
    NAME="Debian GNU/Linux"
    VERSION_ID="12"
    VERSION="12 (bookworm)"
    VERSION_CODENAME=bookworm
    ID=debian
    HOME_URL="https://www.debian.org/"
    SUPPORT_URL="https://www.debian.org/support"
    BUG_REPORT_URL="https://bugs.debian.org/"
    ```

2. Install `pipx` - [more details on pipx](https://pipx.pypa.io/stable/)
   ```
   $ sudo apt update && sudo apt install -y pipx
   ...
   $ pipx --version
   1.1.0
   $
   ```

3. Install ansible with `pipx install ansible` command:
   ```
   $ pipx install ansible
   ...
   $
   ```
4. Verify that ansible is successfully installed with `ansible --version`, `ansible-community --version` and `pipx list` commands:
   ```
   $ ansible --version
   ansible [core 2.19.11]
     config file = None
     configured module search path = ['/home/clab/.ansible/plugins/modules', '/usr/share/ansible/plugins/modules']
     ansible python module location = /home/clab/.local/pipx/venvs/ansible/lib/python3.11/site-packages/ansible
     ansible collection location = /home/clab/.ansible/collections:/usr/share/ansible/collections
     executable location = /home/clab/.local/bin/ansible
     python version = 3.11.2 (main, Apr  8 2026, 01:58:00) [GCC 12.2.0] (/home/clab/.local/pipx/venvs/ansible/bin/python)
     jinja version = 3.1.6
     pyyaml version = 6.0.3 (with libyaml v0.2.5)
   $ ansible-community --version
   Ansible community version 12.3.0
   $ pipx list
    venvs are in /home/clab/.local/pipx/venvs
    apps are exposed on your $PATH at /home/clab/.local/bin
     package ansible 12.3.0, installed using Python 3.11.2
    - ansible
    - ansible-community
    - ansible-config
    - ansible-console
    - ansible-doc
    - ansible-galaxy
    - ansible-inventory
    - ansible-playbook
    - ansible-pull
    - ansible-test
    - ansible-vault
    - cffi-gen-src
   $
   ```
5. List the installed colllection with `ansible-galaxy collection list` command:
   ```
    $ ansible-galaxy collection list

    # /home/clab/.ansible/collections/ansible_collections
    Collection                               Version
    ---------------------------------------- -------
    ansible.netcommon                        8.5.3
    ansible.utils                            6.0.3
    arista.eos                               12.1.2
    
    # /home/clab/.local/pipx/venvs/ansible/lib/python3.11/site-packages/ansible/_internal/ansible_collections
    Collection                               Version
    ---------------------------------------- -------
    ansible._protomatter                     2.19.11
    
    # /home/clab/.local/pipx/venvs/ansible/lib/python3.11/site-packages/ansible_collections
    Collection                               Version
    ---------------------------------------- -------
    amazon.aws                               10.1.2
    ansible.netcommon                        8.2.0
    ansible.posix                            2.1.0
    ansible.utils                            6.0.0
    ansible.windows                          3.3.0
    arista.eos                               12.0.0
    awx.awx                                  24.6.1
    azure.azcollection                       3.12.0
    check_point.mgmt                         6.7.0
    chocolatey.chocolatey                    1.5.3
    cisco.aci                                2.13.0
    cisco.dnac                               6.43.0
    cisco.intersight                         2.12.0
    cisco.ios                                11.1.1
    cisco.iosxr                              12.1.0
    cisco.meraki                             2.21.9
    cisco.mso                                2.12.0
    cisco.nxos                               11.1.0
    cisco.ucs                                1.16.0
    cloudscale_ch.cloud                      2.5.2
    community.aws                            10.0.0
    community.ciscosmb                       1.0.11
    community.crypto                         3.0.5
    community.digitalocean                   1.27.0
    community.dns                            3.4.1
    community.docker                         4.8.5
    community.general                        11.4.2
    community.grafana                        2.3.0
    community.hashi_vault                    7.1.0
    community.hrobot                         2.7.0
    community.library_inventory_filtering_v1 1.1.5
    community.libvirt                        2.0.0
    community.mongodb                        1.7.10
    community.mysql                          3.16.1
    community.okd                            5.0.0
    community.postgresql                     4.2.0
    community.proxmox                        1.4.0
    community.proxysql                       1.7.0
    community.rabbitmq                       1.6.0
    community.routeros                       3.14.0
    community.sap_libs                       1.5.0
    community.sops                           2.2.7
    community.vmware                         5.10.0
    community.windows                        3.1.0
    community.zabbix                         4.1.1
    containers.podman                        1.18.0
    cyberark.conjur                          1.3.9
    cyberark.pas                             1.0.36
    dellemc.enterprise_sonic                 3.2.0
    dellemc.openmanage                       9.12.3
    dellemc.powerflex                        2.6.1
    dellemc.unity                            2.1.0
    f5networks.f5_modules                    1.39.0
    fortinet.fortimanager                    2.11.0
    fortinet.fortios                         2.4.2
    google.cloud                             1.10.2
    grafana.grafana                          6.0.6
    hetzner.hcloud                           5.4.0
    hitachivantara.vspone_block              4.5.0
    hitachivantara.vspone_object             1.1.1
    ibm.qradar                               4.0.0
    ibm.storage_virtualize                   2.7.4
    ieisystem.inmanage                       3.0.0
    infinidat.infinibox                      1.6.3
    infoblox.nios_modules                    1.8.0
    inspur.ispim                             2.2.4
    junipernetworks.junos                    11.0.0
    kaytus.ksmanage                          2.0.0
    kubernetes.core                          6.2.0
    kubevirt.core                            2.2.3
    lowlydba.sqlserver                       2.7.0
    microsoft.ad                             1.10.0
    microsoft.iis                            1.1.0
    netapp.cloudmanager                      21.24.0
    netapp.ontap                             23.2.0
    netapp.storagegrid                       21.15.0
    netapp_eseries.santricity                1.4.1
    netbox.netbox                            3.21.0
    ngine_io.cloudstack                      2.5.0
    openstack.cloud                          2.5.0
    ovirt.ovirt                              3.2.1
    purestorage.flasharray                   1.40.0
    purestorage.flashblade                   1.22.0
    ravendb.ravendb                          1.0.4
    splunk.es                                4.0.0
    telekom_mms.icinga_director              2.5.0
    theforeman.foreman                       5.7.0
    vmware.vmware                            2.6.0
    vmware.vmware_rest                       4.9.0
    vultr.cloud                              1.13.0
    vyos.vyos                                6.0.0
    wti.remote                               1.0.10
    $
   ```
